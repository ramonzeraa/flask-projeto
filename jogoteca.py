...
def novo():
    if 'usuario_logado' not in session or session ['usuario_logado'] == None:
        return redirect(url_for('login', proxima=url_for('novo')))#query string
...
def criar():
    ...
    lista.append(jogo)
    return redirect(url_for('index'))#função que instancia a pagina


@app.route('/login')
def login():
    proxima = request.args.get('proxima')#recuperando dados passados pela query string
